...
import leslie.org.leslie.client.vacation.VacationTablePage.Table;
...
@Data(VacationAppointmentTablePageData.class)
public class VacationTablePage extends AbstractPageWithTable<Table> {
	...
	protected void execLoadData(SearchFilter filter) {
		importPageData(BEANS.get(IVacationAppointmentService.class).getVacationAppointmentTableData(filter));
	}
		...
		public class CancelVacationRequestMenu extends AbstractMenu {
			...
			protected void execAction() {
				...
				if (MessageBoxes.showDeleteConfirmationMessage(TEXTS.get("Vacations"), display)) {
					BEANS.get(IVacationAppointmentService.class).delete(getIdColumn().getSelectedValue());
					reloadPage();
				}
			}
		}

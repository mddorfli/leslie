package leslie.org.leslie.client.permission;
...
import leslie.org.leslie.client.permission.PermissionTablePage.Table.AssignToRoleMenu;
...
public class PermissionTablePage extends AbstractPageWithTable<PermissionTablePage.Table> {
	...
	if (getRoleNr() != null) {
		...
		if (IGlobalPermission.class.isAssignableFrom(clazz)) {
		    try {
			// only list permissions with zero-argument constructors
			// (i.e. global)
			Integer[] levels = null;
			Constructor<? extends Permission> zeroArgConstructor = clazz.getConstructor();
			Permission permissionInstance = zeroArgConstructor.newInstance();
			List<Integer> levelList = ((BasicHierarchyPermission) permissionInstance).getValidLevels();
			...
			logger.error("Could not instantiate permission " + clazz.getName(), e);
		    }
		}
	    }

...
public class MappingUtility {
	...
	public static void exportFormData(AbstractFormData fromFormData, Object toEntity) {
		for (Class<?> clazz = toEntity.getClass(); clazz
				.isAnnotationPresent(MappedClass.class); clazz = clazz.getSuperclass()) {
	...
	public static void importTableRowData(Object fromEntity, AbstractTableRowData toRowData) {
		for (Class<?> clazz = fromEntity.getClass(); clazz
			...
			for (Field field : clazz.getDeclaredFields()) {
				...
				Object value = getProperty(fromEntity, field.getName(), field.getType());
				String mappedFieldName = annotation.pageFieldName().isEmpty()
						? field.getName()
						: annotation.pageFieldName();
	...
	private static void setProperty(Object object, String fieldName, Class<?> fieldType, Object value) {
		...
		try {
			...
			// cast the argument to the correct type if necessary
			Object argValue;
			...
			// try to find a matching setter method
			Method setter = null;
			try {
				setter = object.getClass().getMethod(setterName, argType);

			} catch (NoSuchMethodException e) {
				if (double.class.equals(argType) || Double.class.equals(argType)
						|| float.class.equals(argType) || Float.class.equals(argType)) {
					// if this fails, accept defeat and throw the exception
					setter = object.getClass().getMethod(setterName, BigDecimal.class);
					// if it succeeds, convert the argument to a BigDecimal
					if (argValue != null && argValue instanceof Double) {
						argValue = new BigDecimal((Double) argValue);
					} else if (argValue != null && argValue instanceof Float) {
						argValue = new BigDecimal((Float) argValue);
					}
				} else {
					throw e;
				}
			...
			// prevent setting of null on primitive types
		...
		try {
			...
			if (newType == null) {
				throw new ProcessingException(
						"Could not find {}.{} (or its wrapper or primitive equivalent) for type {}",
						clazz, setterName, argType);
			}
		}
		return newType;
	}
	...
	private static Optional<CustomDataMapping> getCustomMapping(Object entity) {
		...
		MappedClass classMapping = entity.getClass().getAnnotation(MappedClass.class);
